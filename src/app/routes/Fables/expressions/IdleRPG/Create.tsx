...
export default function IdleRpgCreate() {
  ...
  const importFromPack = (pack: IdleRpgPackV1) => {
    ...
    setCreatures(pack.creatures.map((c) => ({
      ...
      iconUrl: c.iconUrl ?? '', introSoundUrl: c.introSoundUrl ?? '', introSoundVolumePercent: String((c as any).introSoundVolumePercent ?? 100), introSoundFadeInMs: String((c as any).introSoundFadeInMs ?? 0), introSoundFadeOutMs: String((c as any).introSoundFadeOutMs ?? 0), backgroundImageUrl: c.backgroundImageUrl ?? '', bossBattleMusicUrl: c.bossBattleMusicUrl ?? '', bossBattleMusicVolumePercent: String((c as any).bossBattleMusicVolumePercent ?? 100), bossBattleMusicFadeInMs: String((c as any).bossBattleMusicFadeInMs ?? 0), bossBattleMusicFadeOutMs: String((c as any).bossBattleMusicFadeOutMs ?? 0), tags: c.tags?.join(', ') ?? '',
    ...
    setItems(pack.items.map((i) => ({
      ...
      iconUrl: i.iconUrl ?? '', colorHex: (i as any).colorHex ?? '', animationUrl: i.animationUrl ?? '',
  ...
  const fillWithExampleData = () => {
    ...
    setAbilities(ex.abilities.map((a) => ({
      ...
      requiredItemTagsAny: (a as any).requiredItemTagsAny ?? '',
      ...
      reactiveTriggerTiming: (a as any).reactiveTriggerTiming ?? 'on_incoming_cast',
      reactivePriority: (a as any).reactivePriority ?? '0',
      reactiveMaxTriggersPerTurn: (a as any).reactiveMaxTriggersPerTurn ?? '',
    })))
    setStatusEffects([])
    ...
    setCreatures(ex.creatures.map((c) => ({
      ...
      bossBattleMusicUrl: (c as any).bossBattleMusicUrl ?? '',
